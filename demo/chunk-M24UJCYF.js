import"./chunk-FBFWB55K.js";var n=`import { Component, ViewEncapsulation, computed, signal, viewChild } from '@angular/core'\r
...
\r
    readonly type = signal<'component' | 'directive'>('component')\r
\r
    readonly disabled = signal(false)\r
    readonly autoReset = signal(false)\r
    readonly clickable = signal(true)\r
    readonly maxFiles = signal<number | undefined>(1)\r
\r
    private readonly timeout = computed(() => this.autoReset() ? 5000 : undefined)\r
\r
    readonly config = computed(() => {\r
        const cfg: DropzoneConfig = {\r
            clickable: this.clickable(),\r
            maxFiles: this.maxFiles()\r
        }\r
        const timeout = this.timeout()\r
        cfg.autoReset = timeout\r
        cfg.errorReset = timeout\r
        cfg.cancelReset = timeout\r
        return cfg\r
    })\r
\r
...
\r
    resetDropzone(): void {\r
        ...
        } else if (this.type() === 'component' && componentRef?.directiveRef()) {\r
